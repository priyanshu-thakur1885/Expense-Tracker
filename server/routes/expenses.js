...
router.post("/", authenticateToken, async (req, res) => {
  try {
    const { item, amount, category, foodCourt, description, tags, date } =
      req.body;
    ...
    if (updatedBudget) {
      const percentage =
        (updatedBudget.currentSpent / updatedBudget.monthlyLimit) * 100;
      ...
      if (percentage >= 100) {
        ...
        if (io) {
          io.to(req.user._id.toString()).emit("budgetExceeded", {
            ...
            exceededAmount:
              updatedBudget.currentSpent - updatedBudget.monthlyLimit,
          });
        }
      }
    }
  ...
  try {
    ...
    switch (period) {
      ...
      case "range":
        ...
        startDate = new Date(qsStartDate);
        endDate = new Date(qsEndDate);
    ...
    if (period === "date") {
      ...
      console.log(
        "All expense dates:",
        allExpenses.map((exp) => exp.date.toISOString().split("T")[0])
      );
      ...
      console.log(
        "Found expenses for date",
        targetDateStr,
        ":",
        expenses.length
      );
      console.log(
        "Filtered expense dates:",
        expenses.map((exp) => exp.date.toISOString().split("T")[0])
      );
    } else {
    ...
    expenses.forEach((expense) => {
      ...
      categoryStats[expense.category] =
        (categoryStats[expense.category] || 0) + expense.amount;

      // Food court statistics
      foodCourtStats[expense.foodCourt] =
        (foodCourtStats[expense.foodCourt] || 0) + expense.amount;
...
router.get("/insights", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (budget) {
      if (todaySpent > budget.dailyLimit)
        recommendations.push("You've exceeded your daily limit!");
      if (totalSpent > budget.monthlyLimit)
        recommendations.push("You've exceeded your monthly budget!");
      if (todaySpent < budget.dailyLimit * 0.5)
        recommendations.push("Great! You're below half of your daily limit.");
    }
