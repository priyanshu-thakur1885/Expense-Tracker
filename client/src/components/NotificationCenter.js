...
const NotificationCenter = () => {
  ...
  const totalUnreadCount =
    unreadCount + adminNotifications.filter((n) => !n.isRead).length;
  ...
  useEffect(() => {
    const handleBudgetExceeded = (event) => {
      ...
      const notification = {
        ...
        message: `Your monthly budget has been exceeded by ₹${exceededAmount.toFixed(
          2
        )}! You've spent ₹${spent.toFixed(2)} out of ₹${limit.toFixed(
          2
        )} (${percentage.toFixed(1)}%). Please review your spending.`,
        severity: "critical",
                    ...
                    onClick={() => {
                      ...
                      if (userId) {
                        localStorage.removeItem(
                          `welcomeNotificationShown_${userId}`
                        );
                      }
                    }}
